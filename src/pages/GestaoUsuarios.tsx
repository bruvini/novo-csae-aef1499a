...
import { collection, getDocs, updateDoc, doc, deleteDoc, arrayUnion, query, where, Timestamp, addDoc } from 'firebase/firestore';
...
const GestaoUsuarios = () => {
  ...
  const filterUsuarios = (users: Usuario[], tab: string, term: string = '') => {
    ...
    if (term) {
      ...
      filtered = filtered.filter(user => 
        ...
        (user.dadosProfissionais.lotacao && user.dadosProfissionais.lotacao.toLowerCase().includes(lowerTerm))
      );
    }
  ...
  const createLogEntry = (usuario: Usuario, acao: Log['acao'], justificativa?: string): Log => {
    return {
  ...
  const deleteUser = async () => {
    ...
    try {
      ...
      // Atualizar estado local
      const updatedUsuarios = usuarios.filter(user => user.id !== selectedUser.id);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container max-w-screen-xl mx-auto my-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-csae-green-700">Gestão de Usuários</h1>
          <p className="text-gray-600 mt-2">
            Gerencie os acessos à plataforma, aprovando novos usuários ou revogando acessos existentes.
          </p>
        </div>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                {loading ? (
                  ...
                  <p className="text-center py-8 text-gray-500">Não há usuários aguardando aprovação.</p>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {usuariosFiltrados.map((usuario) => (
                        <TableRow key={usuario.id}>
                          <TableCell className="font-medium">{usuario.dadosPessoais.nomeCompleto}</TableCell>
                          <TableCell>{formatDate(usuario.dataCadastro)}</TableCell>
                          <TableCell>{usuario.dadosProfissionais.formacao}</TableCell>
                          <TableCell>{usuario.dadosProfissionais.lotacao || 'N/A'}</TableCell>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                {loading ? (
                  ...
                  <p className="text-center py-8 text-gray-500">Não há usuários ativos no momento.</p>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {usuariosFiltrados.map((usuario) => (
                        <TableRow key={usuario.id}>
                          <TableCell className="font-medium">{usuario.dadosPessoais.nomeCompleto}</TableCell>
                          <TableCell>{formatDate(usuario.dataAprovacao)}</TableCell>
                          <TableCell>{formatDate(usuario.dataUltimoAcesso) || 'Nunca acessou'}</TableCell>
                          <TableCell>{usuario.dadosProfissionais.formacao}</TableCell>
                          <TableCell>{usuario.dadosProfissionais.lotacao || 'N/A'}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
            ...
            <Card>
              <CardContent className="p-6">
                ...
                {loading ? (
                  ...
                  <p className="text-center py-8 text-gray-500">Não há usuários revogados ou recusados.</p>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {usuariosFiltrados.map((usuario) => (
                        <TableRow key={usuario.id}>
                          <TableCell className="font-medium">{usuario.dadosPessoais.nomeCompleto}</TableCell>
                          <TableCell>{formatDate(usuario.dataRevogacao)}</TableCell>
      ...
      <Dialog open={isApproveDialogOpen} onOpenChange={setIsApproveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está prestes a conceder acesso para {selectedUser?.dadosPessoais.nomeCompleto}.
          ...
          <div className="space-y-2 py-2">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-600">{selectedUser?.dadosProfissionais.formacao}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-medium">Lotação:</p>
              <p className="text-sm text-gray-600">{selectedUser?.dadosProfissionais.lotacao || 'Não informada'}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-medium">Atua na Secretaria Municipal de Saúde:</p>
              <p className="text-sm text-gray-600">{selectedUser?.dadosProfissionais.atuaSMS ? 'Sim' : 'Não'}</p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsApproveDialogOpen(false)}>Cancelar</Button>
            <Button onClick={approveUser} className="bg-csae-green-600 hover:bg-csae-green-700">Confirmar aprovação</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está recusando o acesso de {selectedUser?.dadosPessoais.nomeCompleto}.
      ...
      <Dialog open={isRevokeDialogOpen} onOpenChange={setIsRevokeDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está revogando o acesso de {selectedUser?.dadosPessoais.nomeCompleto}.
              Por favor, informe o motivo da revogação.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <Textarea
      ...
      <Dialog open={isReactivateDialogOpen} onOpenChange={setIsReactivateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está prestes a reativar o acesso de {selectedUser?.dadosPessoais.nomeCompleto}.
              Esta ação será registrada no sistema.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-2">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium">Motivo da revogação anterior:</p>
              <p className="text-sm text-gray-600">{selectedUser?.motivoRevogacao || 'Não informado'}</p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsReactivateDialogOpen(false)}>Cancelar</Button>
            <Button onClick={reactivateUser} className="bg-csae-green-600 hover:bg-csae-green-700">Confirmar reativação</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está prestes a excluir permanentemente o cadastro de {selectedUser?.dadosPessoais.nomeCompleto}.
              Esta ação não pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <div className="bg-red-50 p-4 rounded-md border border-red-200 my-2">
            <p className="text-red-600 text-sm">
              <strong>Atenção:</strong> A exclusão é permanente e todos os dados associados a este usuário serão removidos do sistema.
              Um log da ação será mantido por questões de auditoria.
            </p>
          </div>
          <DialogFooter>
